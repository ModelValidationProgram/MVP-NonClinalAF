...
MIG_x = c(0.49, 0.01, 0.01)
MIG_y = c(0.01, 0.01, 0.01)
xcline = c("linear", "linear", "V")
ycline = c("linear", "linear", "linear")
demog = c("Estuary", "SS", "SS")
METAPOP_SIDE_x = 10
METAPOP_SIDE_y = 10
...
The 10 genetic architectures were broadly divided into 2 categories: polygenic (many loci of small effect that affect the trait) and oligogenic (few loci of large effect that affect the trait). 

```{r arch base}
arch_level <- factor(c("oliogenic", "polygenic"), ordered=TRUE)
MU_base <- 1e-07
MU_QTL_proportion <- c(0.01, 0.25) # proportion of mutations that are QTNs
SIGMA_QTN_1 <- c(0.4, 0.002)
SIGMA_QTN_2 <- c(0.4, 0.002) #standard deviation of mutational effect size - for trait 2
...
SIGMA_K_2 <- c(0.5, 0.5, 2.0, 0.5, 2.0) # trait 2 is y-axis cline/env
...
Combine to get all 10 levels of architecture
```{r}
...
demog_df$demog <- paste(demog_df$demog_level, demog_df$demog_level_sub, sep="_")
all <- expand.grid(demog = demog_df$demog, arch = arch_df$arch)
...
Add seed 
```{r}
all2$level <- paste(all2$arch, all2$demog, sep="__")
reps <- 1:10
...
Write a script to output the command line argument for a single SliM run
```{r}
