...
lfmm_ridge_temp <- lfmm::lfmm_ridge(Y = scaled.genotype, X = env_temp, K = 4, lambda = 1e-4)
...
lfmm_ridge_temp_test  <- lfmm::lfmm_test(Y = scaled.genotype, X = env_temp, lfmm = lfmm_ridge_temp, calibrate = "gif")

lfmm_ridge_temp_P <- lfmm_ridge_temp_test$calibrated.pvalue

print(c("lfmm.test.ridge$gif", lfmm_ridge_temp_test$gif))
...
LFMM_ridge_temp_log10p <- -log10(as.numeric(lfmm_ridge_temp_P))
#plot(final_df$pos, final_df$LFMM_ridge_0.0_ALL_log10p) 

plot(muts_full$pos_pyslim, LFMM_ridge_temp_log10p)
points(muts_full$pos_pyslim[muts_full$causal], LFMM_ridge_temp_log10p[muts_full$causal], col="blue", pch=20)
