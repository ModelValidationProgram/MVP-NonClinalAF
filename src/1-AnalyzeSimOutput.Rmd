...
rown <- as.numeric(str_remove(vcf_full@fix[,"REF"], "\t"))
...
cbind(position[keepmuts], position_full[keepmuts_full]+1)
...
tail(vcf_full@gt[keepmuts_full,1:10])
  # AHA! I found the problem. IT WAS SHIFTED TO THE LEFT IN FORMAT.
  # THIS MAY BE DUE TO VCF TOOLS, or pyslim!

vcf_full@fix[15:16,]
vcf_full@gt[15:16,1:10]

cbind(vcf@gt[keepmuts,2], vcf_full@gt[keepmuts_full,1])
```
...
if(length(keepmuts) != sum(vcf_full@fix[,"REF"]!="0")){
  print("Error: mutants with MAF >0.01 from sim are a different number from VCF_FULL file")
  break
}



if(length(keepmuts) != dim(muts_df)[1]){
  print("Error: mutants with MAF >0.01 from sim are a different number from VCF file")
  break
}

if (!identical(sort(vcf_muts[keepmuts]), sort(muts_df$mutID))){
  print("Error: mutations not lining up")
  break
}
...
for (i in 1:nrow(muts_df)){
  ind = grep(muts_df$mutID[i], vcf_full@fix[,"REF"])
  if (length(ind)==1){
...
```{r}
# Sanity Check G_subset and G_full_subset give same genotypes
# First individual (column)
# adaptive alleles in G_full_subset given by which(rown!=0)



cbind(G_subset[,1], G_full_subset[which(rown!=0),1])


G_subset[,1:2]
G_full_subset[which(rown!=0),1:2]

# Not lining up
dim(G[keepmuts,])
dim(G_full[which(rown!=0),])

cbind(G[keepmuts,1], G_full[which(rown!=0),1])
