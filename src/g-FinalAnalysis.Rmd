...
```{r cars}
out.df <- read.table(file = "summary_20220117.txt", header=TRUE)
head(out.df)
tail(out.df)

load("src/0b-final_params-20220117.RData")
sims.df <- final
head(sims.df)
...
hist(final.df$X....RDA1_temp_cor)


plot(final.df$X....RDA1_temp_cor, final.df$RDA_RDA1Loading_cor_tempEffect)

hist(final.df$RDA_RDA1Loading_cor_tempEffect)

final.df[which(final.df$RDA_RDA1Loading_cor_tempEffect < -0.9),]
#1231942
#1232405

hist(final.df$RDA_RDA2Loading_cor_salEffect)
final.df[which(final.df$RDA_RDA1Loading_cor_salEffect < -0.9),]
#1231267
#1232407
```
