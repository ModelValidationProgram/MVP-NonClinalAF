...
Accuracy of RDA prediction maxed out by cor (phenotyp, Env)
...
## Env 2 trait
#####################
plot(abs(final.df$cor_PC1_sal), final.df$RDA_cor_RDA20000salpredict_salPhen_structcorr - final.df$RDA_cor_RDA20000salpredict_salPhen)
## What drives the change in performance with structure correction?
...
The ironic thing is that the phenotypic prediction from the RDA is still accurate, and it is affected more by the degree that the phenotype is correlated with the environment, which is driven by demography in the simulation. 
...
# Seeds from Estuary sim, mod poly with high performance
  ...
  i5 <- final.df %>% 
    ...
    arrange(desc(RDA_RDAmutpred_cor_salEffect), desc(RDA_RDAmutpred_cor_tempEffect)) %>%
    head(10) %>% 
  ...
  # Seeds from Estuary sim, mod poly with LOW performance
  i6 <- final.df %>% 
    filter(demog_level=="Est-Clines", arch_level== "mod.\npolygenic") %>% 
    arrange((RDA_RDAmutpred_cor_tempEffect), (RDA_RDAmutpred_cor_salEffect)) %>%
    head(10) %>% 
    select(level,seed,RDA_RDAmutpred_cor_salEffect, RDA_RDAmutpred_cor_tempEffect,
           RDA_cor_RDA20000salpredict_salPhen, RDA_cor_RDA20000temppredict_tempPhen)
