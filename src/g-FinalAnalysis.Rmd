...
library(plyr)
#library(tidyverse)
outputs <- "figures_20220428_20220726/"
```
...
#for (i in c(1,16)){ #unhash to run
#  file <- paste0("cp sim_output_20220428/",final.df$seed[i],"* examples_20220428_20220726/")
#  system(file)
#}
levels(final.df$demog_level)

figs <- which(final.df$arch_level=="mod-polygenic" & final.df$demog_level=="Est-Clines" & final.df$ispleiotropy==1)

final.df$seed[which(final.df$arch_level=="mod-polygenic" & final.df$demog_level=="Est-Clines" & final.df$ispleiotropy==1 & final.df$SIGMA_K_2==0.5 & final.df$demog_level_sub=="N-equal_m-constant")]

#for (i in figs){ #unhash to run
# file <- paste0("cp sim_output_20220428/",final.df$seed[i],"_pdf_heatmaps.pdf examples_20220428_20220726/")
#  system(file)
#   file <- paste0("cp sim_output_20220428/",final.df$seed[i],"_pdf_8phen-env_af-env.pdf examples_20220428_20220726/")
#  system(file)
#  file <- paste0("cp sim_output_20220428/",final.df$seed[i],"_pdf_5RDA.pdf examples_20220428_20220726/")
#    system(file)
#}

i = 1231214#1232792#1232117#1231441 #seed example for publication
system(paste0("cp sim_output_20220428/",i,"*.pdf figures_20220428_20220726/"))
...
`rocket` for arch_level scale_color_viridis(name="Genic level", discrete=TRUE, option="rocket", begin=0, end=0.9)
...
n1 <- ggplot(forscatter) +   geom_point(aes(x=subsamp_corr_phen_temp,
  ...
  ylab("Prediction accuracy\nCor(RDA prediction,\nTemp. phenotype)") + ggtitle("D) Accuracy of RDA prediction\n     (Temp. trait, no structure correction)")
...
n2<-  ggplot(forscatter) +   geom_point(aes(x=subsamp_corr_phen_sal,
  ...
  ylab("Prediction accuracy\nCor(RDA prediction,\nEnv2 phenotype)")  +
  ggtitle("E) Accuracy of RDA prediction\n     (Env2 trait, no structure correction)")
n2  
